...
[Tooltip("The point representing the source of target-detection raycasts for the enemy AI")]
        ...
        {
            // Roguelike: Force target loss if the target is invisible (checked every frame)
            if (KnownDetectedTarget != null)
            {
                var targetInvis = KnownDetectedTarget.GetComponentInParent<AgileInvisibility>();
            ...
            foreach (Actor otherActor in m_ActorsManager.Actors)
            {
                if (otherActor.Affiliation != actor.Affiliation)
                {
                    ...
                    {
                        // Roguelike Invisibility Check
                        var invisibility = otherActor.GetComponent<AgileInvisibility>();
